...
def question3(dataframe):
    ...
    print(f"Proportion of male/female {total_male/total_female:.2f}\n")
...
def question4(dataframe):
    ...
    print('Total male: ',   total_male)
    print('Total female: ', total_female)
    print(f"Proportion of female/male {total_female/total_male:.2f}\n")
...
def question9(dataframe):
    df = dataframe.withColumn('self-employed', (
                    F.when(F.col('workclass').rlike('Self-emp-not-inc'), 'self-employed')\
    ...
    df.where(F.col('self-employed').rlike('self-employed'))\
      ...
      .show()
    #check each result
    most_commom_ed = dataframe.filter(F.col('workclass').rlike('Self-emp-not-inc')| F.col('workclass').rlike('Self-emp-inc'))\
    ...
    most_commom_race = dataframe.filter(F.col('workclass').rlike('Self-emp-not-inc')| F.col('workclass').rlike('Self-emp-inc'))\
                                .groupBy(F.col('race'))\
    ...
    most_commom_sex = dataframe.filter(F.col('workclass').rlike('Self-emp-not-inc')| F.col('workclass').rlike('Self-emp-inc'))\
                               .groupBy(F.col('sex'))\
...
def question10(dataframe):
    data = dataframe.withColumn('married_status', (
        F.when(F.col('marital-status').rlike('Married-civ-spouse'),    'married')\
         .when(F.col('marital-status').rlike('Married-AF'),            'married')\
         .when(F.col('marital-status').rlike('Married-spouse-absent'), 'married')\
         .otherwise('not_married')
    ))
